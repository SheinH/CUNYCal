...
namespace CourseScanner {
    ...
    auto scanCourseTimes(const std::string::const_iterator &stringStart, const std::string::const_iterator &stringEnd);

	auto smatchToCourse(const std::smatch &match);

	std::vector<Course> scanCourses(const std::string &input);

	bool isValidCourseString(const std::string &input);
	/*
	 */
}
